...
work_dir = f"./work_dirs/timesformer-default-divst-v{ver}-{frame_type}-random-crop-sequential"

file_client_args = dict(io_backend="disk")

train_pipeline = [
    dict(type="DecordInit", **file_client_args),
    dict(type="SampleFrames", clip_len=1, frame_interval=32, num_clips=1),
...
val_pipeline = [
    dict(type="DecordInit", **file_client_args),
    dict(type="SampleFrames", clip_len=1, frame_interval=32, num_clips=1, test_mode=True),
    dict(type="DecordDecode"),
...
optim_wrapper = dict(
    optimizer=dict(type="SGD", lr=0.005, momentum=0.9, weight_decay=1e-4, nesterov=True),
    paramwise_cfg=dict(
        custom_keys={
...
param_scheduler = [dict(type="MultiStepLR", begin=0, end=15, by_epoch=True, milestones=[5, 10], gamma=0.1)]
