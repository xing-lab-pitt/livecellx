...
def convert(config_path, output_config_path):
    ...
    if origin_dataset_type != "VideoDataset":
        ...
        cfg.ann_file_train = "data/kinetics400/kinetics400_train_list_rawframes.txt"
        cfg.ann_file_val = "data/kinetics400/kinetics400_val_list_rawframes.txt"
        cfg.ann_file_test = "data/kinetics400/kinetics400_val_list_rawframes.txt"
    ...
    cfg.model.data_preprocessor = dict(type="ActionDataPreprocessor", **dict(preprocess_cfg))
    cfg.pop("img_norm_cfg")

    if (cfg.model.test_cfg is not None) and ("average_clips" in cfg.model.test_cfg):
    ...
    if origin_dataset_type == "VideoDataset":
        ...
        for pipeline in pipelines:
            new_pipeline = [trans for trans in pipeline if trans.type not in ["Normalize", "Collect", "ToTensor"]]
        ...
        for pipeline in pipelines:
            new_pipeline = [
                trans for trans in pipeline if trans.type not in ["RawFrameDecode", "Normalize", "Collect", "ToTensor"]
            ]
    ...
    val_batchsize = cfg.data.val_dataloader.videos_per_gpu if "val_dataloader" in cfg.data else cfg.data.videos_per_gpu
    cfg.data.val.update(
        dict(
            type=cfg.dataset_type,
    ...
    test_batchsize = (
        cfg.data.test_dataloader.videos_per_gpu if "test_dataloader" in cfg.data else cfg.data.videos_per_gpu
    )
    ...
    if "warmup" in cfg.lr_config:
        warmup_ratio = 0.1 if "warmup_ratio" not in cfg.lr_config else cfg.lr_config.warmup_ratio
    ...
    if cfg.lr_config.policy == "step":
        cfg.param_scheduler.append(
            dict(
                ...
                gamma=0.1 if "gamma" not in cfg.lr_config else cfg.lr_config.gamma,
            )
        )
    ...
    cfg.env_cfg = dict(
        ...
        mp_cfg=dict(mp_start_method=cfg.mp_start_method, opencv_num_threads=cfg.opencv_num_threads),
