...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="lr",
        choices=["lr", "momentum"],
        help="The parameter to visualize its change curve, choose from" '"lr" and "momentum". Defaults to "lr".',
    )
    parser.add_argument(
        "-d",
        "--dataset-size",
    ...
    parser.add_argument("-n", "--ngpus", type=int, default=1, help="The number of GPUs used in training.")
    parser.add_argument("-s", "--save-path", type=Path, help="The learning rate curve plot save path")
    parser.add_argument(
        "--log-level", default="WARNING", help="The log level of the handler and logger. Defaults to " "WARNING."
    )
...
class ParamRecordHook(Hook):
    def __init__(self, by_epoch):
        ...
        self.momentum_list = []
        self.task_id = 0
        self.progress = Progress(BarColumn(), MofNCompleteColumn(), TextColumn("{task.description}"))
...
def main():
    ...
    assert len(data_loader) > 0, "Please decrease batchsize to make sure that " "a epoch at least have one iteration!"
    dataset_info = (
        f"\nDataset infos:"
