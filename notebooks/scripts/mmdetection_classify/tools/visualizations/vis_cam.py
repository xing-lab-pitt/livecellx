...
def parse_args():
    parser = argparse.ArgumentParser(description="MMAction2 GradCAM Visualization")
    parser.add_argument("config", help="test config file path")
    parser.add_argument("checkpoint", help="checkpoint file/url")
    parser.add_argument("video", help="video file/url or rawframes directory")
    parser.add_argument("--use-frames", default=False, action="store_true", help="whether to use rawframes as input")
    parser.add_argument("--device", type=str, default="cuda:0", help="CPU/CUDA device option")
    parser.add_argument(
        "--target-layer-name", type=str, default="backbone/layer4/1/relu", help="GradCAM target layer name"
    )
    ...
    parser.add_argument(
        "--resize-algorithm", default="bilinear", help="resize algorithm applied to generate video & gif"
    )
...
def build_inputs(model: nn.Module, video_path: str, use_frames: bool = False) -> Dict:
    ...
    # prepare data
    if use_frames:
        filename_tmpl = cfg.test_dataloader.dataset.get("filename_tmpl", "img_{:05}.jpg")
...
def _resize_frames(
    ...
    frame_list = [mmcv.imresize(img, (new_w, new_h), interpolation=interpolation) for img in frame_list]
...
def main():
    ...
    if args.out_filename is not None:
        ...
        frame_list = _resize_frames(frame_list, args.target_resolution, interpolation=args.resize_algorithm)
