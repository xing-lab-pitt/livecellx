...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="specify fps value of the output video when using rawframes to " "generate file",
    )
    parser.add_argument(
        "--mode",
        "-m",
    ...
    parser.add_argument(
        ...
        "-r",
        type=float,
        help="video rescale factor, which is useful if the output is too " "large or too small.",
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
...
def make_grid(videos, names, rescale_factor=None):
    ...
    if rescale_factor is not None:
        videos = [[mmcv.imrescale(img, rescale_factor) for img in video] for video in videos]
        ...
        for idx, t in enumerate(self.transforms):
            ...
            if "imgs" in data:
                ...
                if name == "ThreeCrop":
                    ...
                    crop_imgs = [imgs[idx * clip_len : (idx + 1) * clip_len] for idx in range(n_crops)]
                    imgs = np.concatenate(crop_imgs, axis=1)
                    imgs = [img for img in imgs]
                if name == "TenCrop":
                    warnings.warn("TenCrop is not supported, only show one crop")
...
def main():
    ...
    visualizer = ActionVisualizer(vis_backends=vis_backends, save_dir="place_holder")
    ...
    for i, item in zip(range(display_number), dataset):
        ...
        if args.mode == "original":
            ...
            video = make_grid([ori_video, trans_video], ["original", "transformed"], rescale_factor)
            rescale_factor = None
        else:
            video = make_grid(
                [result["imgs"] for result in intermediate_imgs],
        ...
        visualizer.add_datasample(file_id, video, data_sample, fps=args.fps, out_type="video")
        progress_bar.update()
