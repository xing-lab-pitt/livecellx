...
def flow_to_img(raw_flow, bound=20.0):
...
def extract_dense_flow(
    ...
    flow_x_names = [osp.join(dest, flow_tmpl.format("x", ind + start_idx)) for ind in range(len(flow_x))]
    flow_y_names = [osp.join(dest, flow_tmpl.format("y", ind + start_idx)) for ind in range(len(flow_y))]
    ...
    if save_rgb:
        img_names = [osp.join(dest, rgb_tmpl.format(ind + start_idx)) for ind in range(len(frames))]
...
def parse_args():
    ...
    parser.add_argument("--prefix", default="", help="the prefix of input " "videos, used when input is a video list")
    parser.add_argument("--dest", default="", help="the destination to save " "extracted frames")
    parser.add_argument("--save-rgb", action="store_true", help="also save " "rgb frames")
    parser.add_argument("--rgb-tmpl", default="img_{:05d}.jpg", help="template filename of rgb frames")
    parser.add_argument("--flow-tmpl", default="{}_{:05d}.jpg", help="template filename of flow frames")
    parser.add_argument("--start-idx", type=int, default=1, help="the start " "index of extracted frames")
    parser.add_argument("--method", default="tvl1", help="use which method to " "generate flow")
    parser.add_argument("--bound", type=float, default=20, help="maximum of " "optical flow")
...
if __name__ == "__main__":
    ...
    if args.input.endswith(".txt"):
        ...
        for video, dest in zip(videos, dests):
            extract_dense_flow(
                video, dest, args.bound, args.save_rgb, args.start_idx, args.rgb_tmpl, args.flow_tmpl, args.method
            )
    else:
        extract_dense_flow(
            args.input, args.dest, args.bound, args.save_rgb, args.start_idx, args.rgb_tmpl, args.flow_tmpl, args.method
        )
