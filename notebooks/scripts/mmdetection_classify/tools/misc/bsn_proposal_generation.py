...
from mmaction.models.localizers.utils import generate_bsp_feature, generate_candidate_proposals
...
def generate_features(ann_file, tem_results_dir, pgm_proposals_dir, pgm_features_dir, pgm_features_thread, **kwargs):
    ...
    for tid in range(pgm_features_thread - 1):
        ...
        p.start()
        processes.append(p)
    tmp_video_list = range((pgm_features_thread - 1) * num_videos_per_thread, num_videos)
    p = mp.Process(
        target=generate_bsp_feature,
        args=(
            tmp_video_list,
...
def main():
    ...
    if args.mode == "test":
        generate_proposals(cfg.ann_file_val, tem_results_dir, pgm_proposals_dir, **cfg.pgm_proposals_cfg)
        print("\nFinish proposal generation")
        generate_features(
            cfg.ann_file_val, tem_results_dir, pgm_proposals_dir, pgm_features_dir, **cfg.pgm_features_test_cfg
        )
        print("\nFinish feature generation")

    elif args.mode == "train":
        generate_proposals(cfg.ann_file_train, tem_results_dir, pgm_proposals_dir, **cfg.pgm_proposals_cfg)
        print("\nFinish proposal generation")
        generate_features(
            cfg.ann_file_train, tem_results_dir, pgm_proposals_dir, pgm_features_dir, **cfg.pgm_features_train_cfg
        )
        print("\nFinish feature generation")
