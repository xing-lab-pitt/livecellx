...
def parse_args():
    ...
    parser.add_argument("--num_frames", type=int, default=8, help="number of input frames.")
    parser.add_argument("--shape", type=int, nargs="+", default=[256, 455], help="input image size")
    parser.add_argument("--device", type=str, default="cpu", help="CPU/CUDA device option")
    parser.add_argument("--output_file", type=str, default="stdet.onnx", help="file name of the output onnx file")
...
def main():
    ...
    proposal = torch.Tensor([[22.0, 59.0, 67.0, 157.0], [186.0, 73.0, 217.0, 159.0], [407.0, 95.0, 431.0, 168.0]])
    ...
    input_feed = {"input_tensor": input_tensor.cpu().data.numpy(), "rois": rois.cpu().data.numpy()}
