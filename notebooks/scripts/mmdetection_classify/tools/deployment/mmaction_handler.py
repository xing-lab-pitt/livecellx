...
class MMActionHandler(BaseHandler):
    def initialize(self, context):
        ...
        if not os.path.isfile(mapping_file_path):
            warnings.warn("Missing the label_map.txt file. " "Inference output will not include class name.")
            self.mapping = None
        else:
    ...
    def postprocess(self, data):
        ...
        for video_idx, video_result in enumerate(data):
            ...
            output[video_idx] = {self.mapping[x[0]] if self.mapping else x[0]: float(x[1]) for x in video_result}
