...
args.pose_checkpoint = (
    "https://download.openmmlab.com/mmpose/top_down/hrnet/hrnet_w32_coco_256x192-c78dce93_20200708.pth"  # noqa: E501
)
...
def ntu_pose_extraction(vid, skip_postproc=False):
    ...
    det_results, _ = detection_inference(
        args.det_config, args.det_checkpoint, frame_paths, args.det_score_thr, device=args.device, with_score=True
    )

    if not skip_postproc:
        det_results = ntu_det_postproc(vid, det_results)
    pose_results, _ = pose_inference(args.pose_config, args.pose_checkpoint, frame_paths, det_results, args.device)

    anno = dict()
    anno["keypoint"] = np.stack([pose["keypoints"].astype(np.float32) for pose in pose_results], axis=1)
    anno["keypoint_score"] = np.stack([pose["keypoint_scores"] for pose in pose_results], axis=1)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate Pose Annotation for a single NTURGB-D video")
