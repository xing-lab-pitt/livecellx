...
def get_names_and_labels(
    data_path: str, task: str, benchmark: str, ignored_samples: Optional[List[str]] = None
) -> Tuple:
    train_names = []
    train_labels = []
    val_names = []
...
def gendata(data_path: str, out_path: str, ignored_sample_path: Optional[str] = None, task: str = "ntu60") -> None:
    split = dict()
    ...
    for benchmark in benchmarks:
        train_names, train_labels, val_names, val_labels = get_names_and_labels(
            data_path, task, benchmark, ignored_samples
        )
    ...
    for i, s in enumerate(names):
        ske = read_xyz(osp.join(data_path, s), max_body=max_body_kinect, num_joint=num_joint).astype(np.float16)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Pose Annotation for NTURGB-D raw skeleton data")
    parser.add_argument(
        "--data-path", type=str, help="raw skeleton data path", default="../../../data/ntu60/nturgb+d_skeletons/"
    )
    parser.add_argument("--ignored-sample-path", type=str, default="NTU_RGBD_samples_with_missing_skeletons.txt")
    parser.add_argument("--out-folder", type=str, default="../../../data/skeleton/")
    ...
    gendata(args.data_path, args.out_folder, args.ignored_sample_path, args.task)
