...
for k, v in annotation.items():
    if k + ".mp4" not in events:
        print(f"video {k[:11]} has not been downloaded " f"or the event clip {k} not generated")
        continue
    ...
    for subaction_id, subaction_anno in v.items():
        ...
        try:
            ...
            print(f"Trimming of the Subaction {subaction_name} of Event " f"{k} Failed", flush=True)
