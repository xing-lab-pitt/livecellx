...
def parse_directory(path, rgb_prefix="img_", flow_x_prefix="flow_x_", flow_y_prefix="flow_y_", level=1):
    ...
    for i, frame_dir in enumerate(frame_dirs):
        ...
        if num_x != num_y:
            raise ValueError(
                f"x and y direction have different number " f"of flow images in video directory: {frame_dir}"
            )
        if i % 200 == 0:
...
def parse_args():
    ...
    parser.add_argument(
        "--mode", type=str, choices=["frames", "videos"], help="generate file list for frames or videos"
    )
...
if __name__ == "__main__":
    ...
    def parse_line(line):
        ...
        for tag in tags:
            ...
            all_tags.setdefault(category, []).append(tag_cates[category].index(tag))
    ...
    if args.mode == "frames":
        result = [dict(frame_dir=k[0], total_frames=parse_result[k[0]][1], label=k[1]) for k in data_list]
