...
def construct_video_filename(item, trim_format, output_dir):
    ...
    basename = "%s_%s_%s.mp4" % (youtube_id, trim_format % start_time, trim_format % end_time)
...
def download_clip_wrapper(item, trim_format, tmp_dir, output_dir):
    ...
    youtube_id, start_time, end_time = item
    downloaded, log = download_clip(youtube_id, output_filename, start_time, end_time, tmp_dir=tmp_dir)
...
def main(input_csv, output_dir, trim_format="%06d", num_jobs=24, tmp_dir="/tmp/hvu"):
    ...
    if num_jobs == 1:
        status_lst = []
        for item in dataset:
            status_lst.append(download_clip_wrapper(item, trim_format, tmp_dir, output_dir))
    else:
        status_lst = Parallel(n_jobs=num_jobs)(
            delayed(download_clip_wrapper)(item, trim_format, tmp_dir, output_dir) for item in dataset
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "input_csv",
        type=str,
        help=("CSV file containing the following format: " "Tags, youtube_id, time_start, time_end"),
    )
