...
def parse_args():
    ...
    parser.add_argument(
        "--data-list", help="video list of the dataset, the format should be " "`frame_dir num_frames output_file`"
    )
    parser.add_argument(
        "--frame-interval", type=int, default=16, help="the sampling frequency of frame in the untrimed video"
    )
    parser.add_argument("--modality", default="RGB", choices=["RGB", "Flow"])
    parser.add_argument("--ckpt", help="checkpoint for feature extraction")
    parser.add_argument("--part", type=int, default=0, help="which part of dataset to forward(alldata[part::total])")
...
def main():
    ...
    model_cfg = dict(
        type="Recognizer2D",
        backbone=dict(type="ResNet", depth=50, in_channels=args.in_channels, norm_eval=False),
        cls_head=dict(
            type="TSNHead",
            num_classes=200,
            in_channels=2048,
    ...
    for item in data:
        ...
        tmpl = dict(
            frame_dir=frame_dir, total_frames=length, filename_tmpl=args.f_tmpl, start_index=0, modality=args.modality
        )
