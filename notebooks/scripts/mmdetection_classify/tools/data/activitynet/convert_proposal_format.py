...
def dump_formatted_proposal(
    video_idx, video_id, num_frames, fps, gts, proposals, tiou, t_overlap_self, formatted_proposal_file
):
    ...
    formatted_proposal_file.write(f"#{video_idx}\n{video_id}\n{num_frames}\n{fps}\n{gts.shape[0]}\n")
    for gt in gts:
    ...
    for i in range(proposals.shape[0]):
        ...
        if best_iou[i] == 0 and best_overlap[i] == 0:
            formatted_proposal_file.write(f"0 0 0 {proposals[i][0]} {proposals[i][1]}\n")
        else:
            formatted_proposal_file.write(
                f"{int(label)} {best_iou[i]} {best_overlap[i]} " f"{proposals[i][0]} {proposals[i][1]}\n"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--ann-file", type=str, default="../../../data/ActivityNet/anet_anno_val.json", help="name of annotation file"
    )
    parser.add_argument(
        "--activity-index-file",
        type=str,
    ...
    parser.add_argument(
        ...
        help="name of proposal file, which is the" "output of proposal generator (BMN)",
    )
    parser.add_argument(
        "--formatted-proposal-file",
        type=str,
        default="../../../anet_val_formatted_proposal.txt",
        help="name of formatted proposal file, which is the" "input of action classifier (SSN)",
    )
...
if __name__ == "__main__":
    ...
    proposal, num_proposals = import_proposals(mmcv.load(args.proposal_file)["results"])
    video_idx = 0
