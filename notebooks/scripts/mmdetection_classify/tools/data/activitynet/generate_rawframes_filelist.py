...
def generate_rawframes_filelist():
    ...
    train_dir_list = [osp.join(train_rawframe_dir, x) for x in os.listdir(train_rawframe_dir)]
    ...
    train_lines = [k + " " + str(training[k][0]) + " " + str(training[k][1]) for k in training]
    val_lines = [k + " " + str(validation[k][0]) + " " + str(validation[k][1]) for k in validation]
