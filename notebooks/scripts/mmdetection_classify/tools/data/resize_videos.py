...
def resize_videos(vid_item):
    ...
    if w > h:
        cmd = (
            f"ffmpeg -hide_banner -loglevel error -i {full_path} "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate the resized cache of original videos")
    parser.add_argument("src_dir", type=str, help="source video directory")
    parser.add_argument("out_dir", type=str, help="output video directory")
    parser.add_argument("--dense", action="store_true", help="whether to generate a faster cache")
    parser.add_argument("--level", type=int, choices=[1, 2], default=2, help="directory level of data")
    parser.add_argument("--remove-dup", action="store_true", help="whether to remove duplicated frames")
    parser.add_argument(
        "--ext", type=str, default="mp4", choices=["avi", "mp4", "webm", "mkv"], help="video file extensions"
    )
    parser.add_argument("--to-mp4", action="store_true", help="whether to output videos in mp4 format")
    parser.add_argument("--scale", type=int, default=256, help="resize image short side length keeping ratio")
