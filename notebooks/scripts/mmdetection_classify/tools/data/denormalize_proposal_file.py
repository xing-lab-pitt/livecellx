...
def process_norm_proposal_file(norm_proposal_file, frame_dict):
    ...
    for idx, norm_proposal in enumerate(norm_proposals):
        ...
        gt = [[int(x[0]), int(float(x[1]) * num_frames), int(float(x[2]) * num_frames)] for x in norm_proposal[2]]

        proposal = [
            [int(x[0]), float(x[1]), float(x[2]), int(float(x[3]) * num_frames), int(float(x[4]) * num_frames)]
            for x in norm_proposal[3]
        ]

        gt_dump = "\n".join(["{} {} {}".format(*x) for x in gt])
        gt_dump += "\n" if len(gt) else ""
        proposal_dump = "\n".join(["{} {:.04f} {:.04f} {} {}".format(*x) for x in proposal])
...
def parse_args():
    ...
    parser.add_argument("dataset", type=str, choices=["thumos14"], help="dataset to be denormalize proposal file")
    parser.add_argument("--norm-proposal-file", type=str, help="normalized proposal file to be denormalize")
    parser.add_argument("--data-prefix", type=str, help="path to a directory where rawframes are held")
