...
def parse_args():
    ...
    parser.add_argument("--rgb-prefix", type=str, default="img_", help="prefix of rgb frames")
    parser.add_argument("--flow-x-prefix", type=str, default="flow_x_", help="prefix of flow x frames")
    parser.add_argument("--flow-y-prefix", type=str, default="flow_y_", help="prefix of flow y frames")
    parser.add_argument("--num-split", type=int, default=3, help="number of split to file list")
    parser.add_argument(
        "--subset", type=str, default="train", choices=["train", "val", "test"], help="subset to generate file list"
    )
    parser.add_argument("--level", type=int, default=2, choices=[1, 2], help="directory level of data")
    parser.add_argument("--format", type=str, default="rawframes", choices=["rawframes", "videos"], help="data format")
    parser.add_argument("--out-root-path", type=str, default="data/", help="root path for output")
    parser.add_argument(
        "--output-format", type=str, default="txt", choices=["txt", "json"], help="built file list format"
    )
    parser.add_argument("--seed", type=int, default=None, help="random seed")
    parser.add_argument("--shuffle", action="store_true", default=False, help="whether to shuffle the file list")
...
def main():
    ...
    if len(splits) > 1:
        for i, split in enumerate(splits):
            file_lists = build_file_list(split, frame_info, shuffle=args.shuffle)
        ...
        if args.subset == "train":
            ...
            raise ValueError(f"subset must be in ['train', 'val', 'test'], " f"but got {args.subset}.")
