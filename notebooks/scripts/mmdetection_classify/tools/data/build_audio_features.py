...
class AudioTools:
    ...
    def __init__(
        self, frame_rate=30, sample_rate=16000, num_mels=80, fft_size=1280, hop_size=320, spectrogram_type="lws"
    ):
    ...
    def _build_mel_basis(self):
        ...
        return librosa.filters.mel(
            self.sample_rate, self.fft_size, fmin=self.fmin, fmax=self.fmax, n_mels=self.num_mels
        )
...
def extract_audio_feature(wav_path, audio_tools, mel_out_dir):
    ...
    if not os.path.exists(mel_filename):
        try:
            ...
            np.save(mel_filename, spectrogram.astype(np.float32), allow_pickle=False)
...
if __name__ == "__main__":
    audio_tools = AudioTools(fft_size=512, hop_size=256)  # window_size:32ms hop_size:16ms
    ...
    files = glob.glob(osp.join(args.audio_home_path, "*/" * args.level, "*" + args.ext))
    ...
    for file in files[part_len * (this_part - 1) : (part_len * this_part) if this_part != num_parts else len(files)]:
        p.apply_async(extract_audio_feature, args=(file, audio_tools, args.spectrogram_save_path))
    p.close()
    p.join()
