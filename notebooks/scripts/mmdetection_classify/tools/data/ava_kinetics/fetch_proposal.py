...
def single_worker(rank, det_lookup, args):
    ...
    detect_sublist = [detect_list[i] for i in range(len(detect_list)) if i % args.num_gpus == rank]

    # register all modules in mmdet into the registries
    register_all_modules()
    model = init_detector(args.config, args.checkpoint, device="cuda:%d" % rank)
...
if __name__ == "__main__":
    ...
    p.add_argument("--config", type=str, default="X-101-64x4d-FPN.py", help="the human detector")
    p.add_argument(
        "--checkpoint",
        type=str,
    ...
    p.add_argument("--picklepath", type=str, default="../../../data/ava_kinetics/kinetics_proposal.pkl")
