...
template = "ffmpeg -ss %d -t %d -accurate_seek -i" " %s -r 30 -avoid_negative_ts 1 %s"
...
def generate_cut_cmds(video_lookup: dict, data_root: str) -> List[str]:
    ...
    for video_id in video_lookup:
        for start, end, video_path in video_lookup[video_id]:
            ...
            new_path = "%s/%s_%06d_%06d.mp4" % (data_root, video_id, start, end)
            cmd = template % (start - start0, end - start, video_path, new_path)
            cmds.append(cmd)
    return cmds
...
if __name__ == "__main__":
    ...
    p.add_argument("--kinetics_list", type=str, help="the datalist of the kinetics700 training videos")
    p.add_argument("--num_workers", type=int, default=-1, help="number of workers used for multiprocessing")
    p.add_argument(
        "--avakinetics_root",
        type=str,
