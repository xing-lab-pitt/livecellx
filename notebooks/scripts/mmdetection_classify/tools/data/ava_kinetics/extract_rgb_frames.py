...
def extract_rgb(video_name, frame_path, video_path):
    ...
    cmd = "ffmpeg -i %s/%s -r 30 -q:v 1 %s/%s" % (video_path, video_name, frame_path, video_id)
    cmd += "/img_%05d.jpg"
    return cmd
...
if __name__ == "__main__":
    ...
    all_cmds = [extract_rgb(video_name, frame_path, video_path) for video_name in os.listdir(video_path)]
