    ...
    if task == "rgb":
        if args.use_opencv:
                    ...
                    if vr_frame is not None:
                        ...
                        if args.new_short == 0:
                            if args.new_width == 0 or args.new_height == 0:
                                ...
                                out_img = mmcv.imresize(vr_frame, (args.new_width, args.new_height))
                        else:
                            if min(h, w) == h:
                        ...
                        warnings.warn("Length inconsistent!" f"Early stop with {i + 1} out of {len(vr)} frames.")
                        break
                run_success = 0
            except Exception:
                run_success = -1
            ...
            if args.new_short == 0:
                ...
                cmd = osp.join(f"denseflow '{full_path}' -b=20 -s=0 -o='{out_full_path}'" f" -ns={args.new_short} -v")
        ...
        if args.new_short == 0:
            ...
            cmd_rgb = osp.join(f"denseflow '{full_path}' -b=20 -s=0 -o='{out_full_path}'" f" -ns={args.new_short} -v")
            cmd_flow = osp.join(
                f"denseflow '{full_path}' -a={method} -b=20 -s=1 -o='{out_full_path}'"  # noqa: E501
                f" -ns={args.new_short} -v"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--task", type=str, default="flow", choices=["rgb", "flow", "both"], help="which type of frames to be extracted"
    )
    parser.add_argument("--level", type=int, choices=[1, 2], default=2, help="directory level of data")
    parser.add_argument("--num-worker", type=int, default=8, help="number of workers to build rawframes")
    parser.add_argument(
        "--flow-type",
        type=str,
        default=None,
    ...
    parser.add_argument("--out-format", type=str, default="jpg", choices=["jpg", "h5", "png"], help="output format")
    parser.add_argument("--ext", type=str, default="avi", choices=["avi", "mp4", "webm"], help="video file extensions")
    parser.add_argument("--mixed-ext", action="store_true", help="process video files with mixed extensions")
    ...
    parser.add_argument(
        "--resume", action="store_true", default=False, help="resume optical flow extraction instead of overwriting"
    )
    parser.add_argument("--use-opencv", action="store_true", help="Whether to use opencv to extract rgb frames")
    parser.add_argument(
        "--input-frames", action="store_true", help="Whether to extract flow frames based on rgb frames"
    )
    parser.add_argument(
        "--report-file",
        type=str,
