...
def construct_video_filename(row, label_to_dir, trim_format="%06d"):
    ...
    given video."""
    basename = "%s_%s_%s.mp4" % (row["video-id"], trim_format % row["start-time"], trim_format % row["end-time"])
...
def download_clip_wrapper(row, label_to_dir, trim_format, tmp_dir):
    ...
    downloaded, log = download_clip(
        row["video-id"], output_filename, row["start-time"], row["end-time"], tmp_dir=tmp_dir
    )
...
def main(input_csv, output_dir, trim_format="%06d", num_jobs=24, tmp_dir="/tmp/kinetics"):
    ...
    if num_jobs == 1:
        ...
        for _, row in dataset.iterrows():
            status_list.append(download_clip_wrapper(row, label_to_dir, trim_format, tmp_dir))
    else:
        status_list = Parallel(n_jobs=num_jobs)(
            delayed(download_clip_wrapper)(row, label_to_dir, trim_format, tmp_dir) for i, row in dataset.iterrows()
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "input_csv",
        type=str,
        help=("CSV file containing the following format: " "YouTube Identifier,Start time,End time,Class label"),
    )
