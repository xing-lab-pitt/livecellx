...
def parse_args():
    ...
    parser.add_argument("--nproc", type=int, default=(cpu_count() - 1 or 1), help="Number of processes to check videos")
    ...
    def __call__(self, results):
        ...
        if results["total_frames"] > 2:
            results["frame_inds"] = np.concatenate(
                [results["frame_inds"], np.random.randint(1, results["total_frames"] - 1, 3)]
            )
...
if __name__ == "__main__":
    ...
    decoder_to_pipeline_prefix = dict(decord="Decord", opencv="OpenCV", pyav="PyAV")
    ...
    data_info_list = [dataset.get_data_info(idx) for idx in range(len(dataset))]
    ...
    if os.path.exists(args.output_file):
        ...
        print(f"Checked {len(dataset)} videos, " f"{num_lines} are corrupted/missing.")
