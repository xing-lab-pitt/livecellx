...
from mmaction.evaluation.functional import get_weighted_score, mean_class_accuracy, top_k_accuracy


def parse_args():
    parser = argparse.ArgumentParser(description="Fusing multiple scores")
    parser.add_argument(
        "--preds", nargs="+", help="list of predict result", default=["demo/fuse/joint.pkl", "demo/fuse/bone.pkl"]
    )
    parser.add_argument(
        "--coefficients", nargs="+", type=float, help="coefficients of each score file", default=[1.0, 1.0]
    )
...
def main():
    ...
    for data_samples in data_sample_list:
        scores = [sample["pred_scores"]["item"].numpy() for sample in data_samples]
        score_list.append(scores)
    labels = [sample["gt_labels"]["item"].item() for sample in data_sample_list[0]]
