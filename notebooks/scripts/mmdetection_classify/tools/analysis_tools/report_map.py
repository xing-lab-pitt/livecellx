...
def parse_args():
    parser = argparse.ArgumentParser(description="Report detection mAP for" "ActivityNet proposal file")
    ...
    parser.add_argument(
        ...
        default="cuhk17_top1",
        choices=["cuhk17_top1"],
        help="the way to assign label for each " "proposal",
    )
    parser.add_argument("--det-output", type=str, default="det_result.json", help="the path to store detection results")
...
def main():
    ...
    anet_detection = ActivityNetLocalization(
        args.gt, args.det_output, tiou_thresholds=np.linspace(0.5, 0.95, 10), verbose=True
    )
