...
def discrete_factor_prediction_score(
    latent_factor_train, ground_truth_train, latent_factor_test, ground_truth_test, cls
):
...
def continuos_factor_prediction_score(
    latent_factor_train, ground_truth_train, latent_factor_test, ground_truth_test, cls
):
    if cls == "LR" or cls == "lr":
...
def calculate_score(dataset_name, model, z_dim, pixel, scale, batch_size, device):
    ...
    return discrete_factor_prediction_score(
        train_semantic_z.reshape(-1, z_dim), train_labels, test_semantic_z.reshape(-1, z_dim), test_labels, "knn"
    )
