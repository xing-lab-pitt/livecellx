...
def inference_recognizer(
    ...
    if isinstance(video, dict):
        ...
        raise RuntimeError(f"The type of argument `video` is not supported: " f"{type(video)}")
    ...
    if input_flag == "audio":
        data = dict(audio_path=video, total_frames=len(np.load(video)), start_index=0, label=-1)
    ...
    if require_grad:
        ...
        # print("data inputs len:", len(data["inputs"]))
        # print("data:", data)
        data["inputs"][0] = data["inputs"][0].float()
        data["inputs"][0].requires_grad_()
        result = model.test_step(data)[0]
    else:
        with torch.no_grad():
            result = model.test_step(data)[0]
    if return_data_and_grad:
        preds = result.pred_score
        score, indices = torch.max(preds, 0)
        score.backward()
        return result, data, data["inputs"][0].grad
    return result


def inference_skeleton(
    model: nn.Module, pose_results: List[dict], img_shape: Tuple[int], test_pipeline: Optional[Compose] = None
) -> ActionDataSample:
...
def detection_inference(
    ...
    if isinstance(det_config, nn.Module):
        ...
        model = init_detector(config=det_config, checkpoint=det_checkpoint, device=device)
    ...
    for frame_path in track_iter_progress(frame_paths):
        ...
        valid_idx = np.logical_and(pred_instance.labels == det_cat_id, pred_instance.scores > det_score_thr)
...
def pose_inference(
    ...
    try:
        ...
        raise ImportError(
            "Failed to import `inference_topdown` and "
    ...
    for f, d in track_iter_progress(list(zip(frame_paths, det_results))):
        pose_data_samples: List[PoseDataSample] = inference_topdown(model, f, d[..., :4], bbox_format="xyxy")
