...
class VAEXperiment(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx, optimizer_idx=0):
        ...
        val_loss = self.model.loss_function(
            *results, M_N=1.0, optimizer_idx=optimizer_idx, batch_idx=batch_idx  # real_img.shape[0]/ self.num_val_imgs,
        )
    ...
    def sample_images(self):
        ...
        vutils.save_image(
            recons.data,
            os.path.join(
                self.logger.log_dir, "Reconstructions", f"recons_{self.logger.name}_Epoch_{self.current_epoch}.png"
            ),
            normalize=True,
            nrow=12,
        )
        ...
        try:
            if self.params["LR_2"] is not None:
                optimizer2 = optim.Adam(
                    getattr(self.model, self.params["submodel"]).parameters(), lr=self.params["LR_2"]
                )
