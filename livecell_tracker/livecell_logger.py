##################
# A logger adapted from dynamo-release
##################
import functools
import logging
import sys
...
class Logger:
    """livecell-X-specific logger that sets up logging for the package."""
...
class LoggerManager:
    ...
    main_logger = Logger("livecell-X")
    temp_timer_logger = Logger("livecell-X-temp-timer-logger")
...
def main_set_level(level):
    set_logger_level("livecell-X", level)
